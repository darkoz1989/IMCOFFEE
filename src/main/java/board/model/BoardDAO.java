...
import board.database.DBConnection;

public class BoardDAO {
	
	//DB접속시 필요한 멤버객체를 선언하고 있다.
	...
	//싱글톤 패턴으로 BoardDAO객체를 하나만 만들어서 리턴하고 있다.
	...
	public int getListCount(String items, String text) {
		...
		try {
			...
			System.out.println("getListCount()에외" + e.getMessage());
			e.printStackTrace();
		}finally {
			try {
				if(rs != null) rs.close();
	...
	public String getLoginName(String id) {
		...
		try {
			...
			System.out.println("getLoginNameNum예외발생 : " + e.getMessage());
			e.printStackTrace();
		}finally {
			try {
				if(rs != null) rs.close();
	...
	public void insertBoard(BoardDTO board)  {
		...
		try {
			//auto increment부분을 다시 1로 재설정을 해주는 부분이다.
			conn = DBConnection.getConnection();		
			pstmt = conn.prepareStatement(sql);
			pstmt.executeUpdate();
			pstmt.close();
	...
	public BoardDTO getBoardByNum(int num, int page) {
		...
		//조회수를 증가시키는 메서드 호출 부분
		updateHit(num);
		
		return board;
	}
	
	//선택된 글의 조회수 증가시키는 메서드
